import React, { useEffect, useState } from "react";

import {
  Input,
  Form,
  Row,
  Col,
  Typography,
  Avatar,
  Button,
  Card,
  Layout,
  Carousel,
  Descriptions,
  Badge,
} from "antd";
import {
  UserOutlined,
  LockOutlined,
  PhoneOutlined,
  LoadingOutlined,
} from "@ant-design/icons";

import { useHistory, Link, useParams } from "react-router-dom";
import { isEmpty, isLoaded, useFirestoreConnect } from "react-redux-firebase";
import { runNotifications } from "../helpers/Notification";
import UploadProfilePicture from "./UploadProfilePicture";
import Navigation from "./Navigation";
import { useSelector, connect } from "react-redux";
import { useFirebase } from "react-redux-firebase";
import { storage } from "../createStore";
const { Title, Text } = Typography;
const { Content } = Layout;
const Post = (props) => {
  const history = useHistory();
  let { id } = useParams();

  useFirestoreConnect([
    {
      collection: "posts",
    },
  ]);
  const post =
    useSelector(({ firestore: { data } }) => data.posts && data.posts[id]) ||
    {};

  console.log(post);

  return (
    <Layout>
      <Navigation />
      <Content style={styles.content}>
        <Title style={{ textAlign: "center" }}>My profile</Title>
        {!isEmpty(post) ? (
          <Row justify="center" style={styles.mainRow}>
            <Col style={{ marginTop: "100px" }} offset={1} md={13} xs={20}>
              <Card>
                <Carousel autoplay>
                  {post.pictures.map((i) => {
                    return (
                      <div>
                        <img
                          style={{ height: "300px", align: "center" }}
                          src={`https://firebasestorage.googleapis.com/v0/b/${process.env.REACT_APP_STORAGE_BUCKET}/o/pets%2F${i}?alt=media`}
                          alt="img"
                        />
                      </div>
                    );
                  })}
                </Carousel>
                <Descriptions
                  title="Pet Info"
                  layout="vertical"
                  bordered
                  style={{ marginTop: "50px" }}
                >
                  <Descriptions.Item label="Product">
                    Cloud Database
                  </Descriptions.Item>
                  <Descriptions.Item label="Billing Mode">
                    Prepaid
                  </Descriptions.Item>
                  <Descriptions.Item label="Automatic Renewal">
                    YES
                  </Descriptions.Item>
                  <Descriptions.Item label="Order time">
                    2018-04-24 18:00:00
                  </Descriptions.Item>
                  <Descriptions.Item label="Usage Time" span={2}>
                    2019-04-24 18:00:00
                  </Descriptions.Item>
                  <Descriptions.Item label="Status" span={3}>
                    <Badge status="processing" text="Running" />
                  </Descriptions.Item>
                  <Descriptions.Item label="Negotiated Amount">
                    $80.00
                  </Descriptions.Item>
                  <Descriptions.Item label="Discount">$20.00</Descriptions.Item>
                  <Descriptions.Item label="Official Receipts">
                    $60.00
                  </Descriptions.Item>
                  <Descriptions.Item label="Config Info">
                    Data disk type: MongoDB
                    <br />
                    Database version: 3.4
                    <br />
                    Package: dds.mongo.mid
                    <br />
                    Storage space: 10 GB
                    <br />
                    Replication factor: 3
                    <br />
                    Region: East China 1<br />
                  </Descriptions.Item>
                </Descriptions>
              </Card>
            </Col>
            <Col style={{ marginTop: "100px" }} offset={1} md={4} xs={20}>
              <Row justify="center" align="middle" style={{ width: "100%" }}>
                <Card style={{}}>
                  <Title level={3}>Contact the owner</Title>
                  <div style={{ display: "inline" }}>
                    <Avatar size={100} src="" />
                    <Title level={4}>Avram Iancu</Title>
                  </div>
                  <Button block>Click to unreveal phonenumber</Button>
                </Card>
              </Row>
            </Col>
          </Row>
        ) : (
          <Row justify="center" style={styles.mainRow}>
            {/* <Text>Loading...</Text> */}
            <LoadingOutlined style={{ fontSize: "250px" }} />
          </Row>
        )}
      </Content>
    </Layout>
  );
};

const styles = {
  content: { minHeight: "100vh" },
  mainRow: {},
};

export default Post;
